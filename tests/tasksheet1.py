...
def test_task_1(verbose):
    ...
    root_finding.func_iter(x0, g, TOLERANCE, MAX_ITERS, verbose=verbose)
    g = lambda x: x + x * np.exp(-x)
    print('\n   g -> x + f(x)')
    root_finding.func_iter(x0, g, TOLERANCE, MAX_ITERS, verbose=verbose)
    print()  
...
def test_task_3(verbose):
    ...
    for x in x0:
        if x != 0:
            root_finding.func_iter(x, g, TOLERANCE, MAX_ITERS, verbose=verbose)
    # with epsilon = 0.0001
    epsilon = 0.0001
...
def test_task_4(verbose):
    ...
    root_finding.bisect(a, b, f, TOLERANCE, verbose=verbose)
    ...
    for bounds in ab:
        root_finding.bisect(bounds[0], bounds[1], f, TOLERANCE, verbose=verbose)
